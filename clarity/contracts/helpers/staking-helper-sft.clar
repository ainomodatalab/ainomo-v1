...
(define-private (get-staker-at-cycle-or-default-by-tx-sender (token principal) (token-id uint) (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool-sft get-staker-at-cycle-or-default token token-id reward-cycle (default-to u0 (contract-call? .ainomo-reserve-pool-sft get-user-id token token-id tx-sender)))
)

(define-private (get-staking-reward-by-tx-sender (token principal) (token-id uint) (target-cycle uint))
  (contract-call? .ainomo-reserve-pool-sft get-staking-reward token token-id (default-to u0 (contract-call? .ainomo-reserve-pool-sft get-user-id token token-id tx-sender)) target-cycle)
)

(define-public (claim-staking-reward-by-tx-sender (token <sft-trait>) (token-id uint) (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool-sft claim-staking-reward token token-id reward-cycle)
)

(define-read-only (get-staking-stats-coinbase (token principal) (token-id uint) (reward-cycle uint))
    { 
        staking-stats: (contract-call? .ainomo-reserve-pool-sft get-staking-stats-at-cycle-or-default token token-id reward-cycle), 
        coinbase-amount: (contract-call? .ainomo-reserve-pool-sft get-coinbase-amount-or-default token token-id reward-cycle)
    }
)
