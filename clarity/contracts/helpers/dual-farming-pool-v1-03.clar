...
(define-public (claim-staking-reward-by-tx-sender (token-trait <ft-trait>) (dual-token-trait <ft-trait>) (target-cycle uint))
  (let
    (
      ...
      (sender tx-sender)
      (claimed (try! (contract-call? .ainomo-reserve-pool claim-staking-reward token-trait target-cycle)))
...
(define-public (claim-staking-reward-by-auto-ainomo (dual-token-trait <ft-trait>) (target-cycle uint))
  (let 
    (
      (dual-token (contract-of dual-token-trait))
      (user-id (unwrap! (contract-call? .ainomo-reserve-pool get-user-id .age000-governance-token .auto-ainomo-v2) ERR-USER-ID-NOT-FOUND))
      (entitled-token (contract-call? .ainomo-reserve-pool get-staking-reward .age000-governance-token user-id target-cycle))
    ...
    (try! (contract-call? .auto-ainomo-v2 claim-and-stake target-cycle))
    (and 
      (> entitled-dual u0)
      (as-contract (try! (contract-call? dual-token-trait transfer-fixed entitled-dual tx-sender .auto-ainomo-v2 none)))
    )
    (ok { to-return: u0, entitled-token: entitled-token, entitled-dual: entitled-dual })
  )
)

(define-public (claim-and-mint-auto-ainomo (dual-token-trait <ft-trait>) (reward-cycles (list 200 uint)))
  (let 
    (
      (claimed (unwrap-panic (claim-staking-reward .age000-governance-token dual-token-trait reward-cycles)))
    )
    (try! (contract-call? .auto-ainomo-v2 add-to-position (fold sum-claimed claimed u0)))
    (ok claimed)
  )
)
