...
(define-private (get-staker-at-cycle-or-default-by-tx-sender (token principal) (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool get-staker-at-cycle-or-default token reward-cycle (default-to u0 (contract-call? .ainomo-reserve-pool get-user-id token tx-sender)))
)

(define-private (get-staking-reward-by-tx-sender (token principal) (target-cycle uint))
  (contract-call? .ainomo-reserve-pool get-staking-reward token (default-to u0 (contract-call? .ainomo-reserve-pool get-user-id token tx-sender)) target-cycle)
)

(define-public (claim-staking-reward-by-tx-sender (token <ft-trait>) (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool claim-staking-reward token reward-cycle)
)

(define-read-only (get-staking-stats-coinbase (token principal) (reward-cycle uint))
    { 
        staking-stats: (contract-call? .ainomo-reserve-pool get-staking-stats-at-cycle-or-default token reward-cycle), 
        coinbase-amount: (contract-call? .ainomo-reserve-pool get-coinbase-amount-or-default token reward-cycle)
    }
)
