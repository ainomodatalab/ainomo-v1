...
(define-public (execute (token <ft-trait>) (amount uint) (memo (optional (buff 16))))
    (let
        (   
            (swapped-to-ainomo (try! (contract-call? .swap-helper-v1-03 swap-helper .token-wstx .age000-governance-token amount none)))      
            (swapped-to-diko (try! (contract-call? .amm-swap-pool swap-helper .age000-governance-token .token-wdiko ONE_8 swapped-to-ainomo none)))      
            (swapped-back
                (/
                    (*
                        (unwrap-panic 
