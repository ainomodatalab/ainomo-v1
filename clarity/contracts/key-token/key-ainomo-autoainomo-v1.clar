...
(define-fungible-token key-ainomo-autoainomo-v1)
...
(define-data-var token-name (string-ascii 32) "key-ainomo-autoainomo-v1")
(define-data-var token-symbol (string-ascii 32) "key-ainomo-autoainomo-v1")
...
(define-read-only (get-overall-balance (who principal))
	(ok (ft-get-balance key-ainomo-autoainomo-v1 who))
)
...
(define-read-only (get-overall-supply)
	(ok (ft-get-supply key-ainomo-autoainomo-v1))
)
...
(define-public (transfer (token-id uint) (amount uint) (sender principal) (recipient principal))
	(let
		...
		(try! (ft-transfer? key-ainomo-autoainomo-v1 amount sender recipient))
...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
	(begin
		...
		(try! (ft-mint? key-ainomo-autoainomo-v1 amount recipient))
		(try! (set-balance token-id (+ (get-balance-or-default token-id recipient) amount) recipient))
...
(define-public (burn (token-id uint) (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(try! (ft-burn? key-ainomo-autoainomo-v1 amount sender))
...
(define-read-only (get-overall-supply-fixed)
	(ok (decimals-to-fixed (ft-get-supply key-ainomo-autoainomo-v1)))
)
...
(define-read-only (get-overall-balance-fixed (who principal))
	(ok (decimals-to-fixed (ft-get-balance key-ainomo-autoainomo-v1 who)))
)
