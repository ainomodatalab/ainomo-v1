...
(define-public (execute (collateral <ft-trait>) (amount uint) (memo (optional (buff 16))))
    (let
        (   
            ...
            (minted-yield-token (get yield-token (try! (contract-call? .collateral-rebalancing-pool-v1 add-to-position .token-wusda .age000-governance-token memo-uint .yield-usda .key-usda-ainomo gross-amount))))
        ...
        (print { object: "flash-loan-user-margin-ainomo-usda", action: "execute", data: gross-amount })
        (ok true)
    )
)
