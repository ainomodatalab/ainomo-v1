...
(define-private (stake-tokens-at-cycle (token-trait <ft-trait>) (user principal) (user-id uint) (amount-token uint) (start-height uint) (lock-period uint))
  (let
    ...
    (unwrap! (contract-call? token-trait transfer-fixed amount-token tx-sender .ainomo-vault none) ERR-TRANSFER-FAILED)
...
(define-private (claim-staking-reward-at-cycle (token-trait <ft-trait>) (user principal) (stacks-height uint) (target-cycle uint))
  (let
    ...
    (and (> to-return u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft token-trait to-return user))) (as-contract (try! (remove-from-balance (contract-of token-trait) to-return))))
...
(map-set approved-contracts .fixed-weight-pool-ainomo true)
