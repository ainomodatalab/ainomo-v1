...
(define-public (add-to-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (max-dy (optional uint)))
    (let
        ...
        (try! (contract-call? token-x-trait transfer-fixed dx sender .ainomo-vault-v1-1 none))
        (try! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault-v1-1 none))
...
(define-public (reduce-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (percent uint))
    (let
        ...
        (as-contract (try! (contract-call? .ainomo-vault-v1-1 transfer-ft-two token-x-trait dx token-y-trait dy sender)))
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
...
(define-public (swap-x-for-y (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (min-dy (optional uint)))
    (let
        ...
        (try! (contract-call? token-x-trait transfer-fixed dx sender .ainomo-vault-v1-1 none))
        (and (> dy u0) (as-contract (try! (contract-call? .ainomo-vault-v1-1 transfer-ft token-y-trait dy sender))))
        (as-contract (try! (contract-call? .ainomo-vault-v1-1 add-to-reserve token-x (- fee fee-rebate))))
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
...
(define-public (swap-y-for-x (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dy uint) (min-dx (optional uint)))
    (let
        ...
        (try! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault-v1-1 none))
        (and (> dx u0) (as-contract (try! (contract-call? .ainomo-vault-v1-1 transfer-ft token-x-trait dx sender))))            
        (as-contract (try! (contract-call? .ainomo-vault-v1-1 add-to-reserve token-y (- fee fee-rebate))))
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
