...
(define-private (get-first-stacks-block-in-reward-cycle (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool get-first-stacks-block-in-reward-cycle .age000-governance-token reward-cycle)
)
...
(define-public (add-to-position (dx uint))
  (let
    ...
    (as-contract (try! (contract-call? .auto-ainomo mint-fixed new-supply sender)))
...
;; @desc dissolves the vault and allows auto-ainomo holders to withdraw $AINOMO unstaked from the vault
;; @desc burn all auto-ainomo held by tx-sender and transfer $AINOMO due to tx-sender
;; @assert contract-owner to set-activated to false before such withdrawal can happen.
;; @assert there are no staking positions (i.e. all $AINOMO are unstaked)
(define-public (reduce-position)
  (let
    (
      ...
      (balance (unwrap! (contract-call? .age000-governance-token get-balance-fixed (as-contract tx-sender)) ERR-GET-BALANCE-FIXED-FAIL))
      (reduce-supply (unwrap! (contract-call? .auto-ainomo get-balance-fixed sender) ERR-GET-BALANCE-FIXED-FAIL))
    ...
    (as-contract (try! (contract-call? .auto-ainomo burn-fixed reduce-supply sender)))
...
(contract-call? .ainomo-vault add-approved-token .auto-ainomo)
