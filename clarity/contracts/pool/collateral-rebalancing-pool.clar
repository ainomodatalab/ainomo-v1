    ...
    (begin
        ...
        (let
            (
                ...
                (bal-y-short-act (if (is-eq bal-x-to-sell u0) u0 (begin (as-contract (try! (contract-call? .ainomo-vault transfer-ft collateral-trait bal-x-to-sell tx-sender))) (as-contract (try! (swap-helper collateral-trait token-trait bal-x-to-sell none))))))                
        ...
        (if (> (is-fixed-weight-pool-v1-01 token-x token-y) u0)           
            ...
            (if (> (is-simple-weight-pool-ainomo token-x token-y) u0)
                (begin
                    ...
                    (ok (get dy (try! (contract-call? .simple-weight-pool-ainomo swap-x-for-y token-x-trait token-y-trait dx min-dy))))
                )
                (if (> (is-from-fixed-to-simple-ainomo token-x token-y) u0)
                    (if (is-eq token-x .token-wstx)
                        (let ((output (get dy (try! (contract-call? .simple-weight-pool-ainomo swap-ainomo-for-y token-y-trait 
                    ...
                    (if (is-eq token-y .token-wstx)
                        ...
                        (let ((output (get dy (try! (contract-call? .fixed-weight-pool-v1-01 swap-wstx-for-y token-y-trait u50000000 
                                ...
                                (get dx (try! (contract-call? .simple-weight-pool-ainomo swap-y-for-ainomo token-x-trait dx none))) none))) min-dy)))))
                            (map-set simple-oracle-resilient-map { token-x: token-x, token-y: .age000-governance-token } (try! (simple-oracle-resilient-internal token-x .age000-governance-token)))
                            (map-set fwp-oracle-resilient-map { token-x: .age000-governance-token, token-y: .token-wstx } (try! (fwp-oracle-resilient-internal .age000-governance-token .token-wstx)))
                            (map-set fwp-oracle-resilient-map { token-x: .token-wstx, token-y: token-y } (try! (fwp-oracle-resilient-internal .token-wstx token-y)))                            
                            (ok output)
...
(define-read-only (get-given-helper (token-x principal) (token-y principal) (dy uint))
    (ok
        (if (> (is-fixed-weight-pool-v1-01 token-x token-y) u0)
            ...
            (if (> (is-simple-weight-pool-ainomo token-x token-y) u0)
                (try! (contract-call? .simple-weight-pool-ainomo get-x-given-y token-x token-y dy))
                (if (> (is-from-fixed-to-simple-ainomo token-x token-y) u0)
                    (try! (contract-call? .fixed-weight-pool-v1-01 get-x-given-y token-x .age000-governance-token u50000000 u50000000 
                        (try! (contract-call? .simple-weight-pool-ainomo get-ainomo-given-y token-y dy)))) 
                    (try! (contract-call? .simple-weight-pool-ainomo get-y-given-ainomo token-x
...
(define-private (simple-oracle-instant-internal (token-x principal) (token-y principal))
    (let 
        (
            (exists (is-some (contract-call? .simple-weight-pool-ainomo get-pool-exists token-x token-y)))
            (pool 
                (if exists
                    (try! (contract-call? .simple-weight-pool-ainomo get-pool-details token-x token-y))
                    (try! (contract-call? .simple-weight-pool-ainomo get-pool-details token-y token-x))
                )
            )
        )
        (asserts! (get oracle-enabled pool) ERR-ORACLE-NOT-ENABLED)
...
(define-private (simple-oracle-resilient-internal (token-x principal) (token-y principal))
    (let 
        (
            (pool 
                (if (is-some (contract-call? .simple-weight-pool-ainomo get-pool-exists token-x token-y))
                    (try! (contract-call? .simple-weight-pool-ainomo get-pool-details token-x token-y))
                    (try! (contract-call? .simple-weight-pool-ainomo get-pool-details token-y token-x))
                )
            )
