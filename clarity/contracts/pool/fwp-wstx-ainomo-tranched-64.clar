...
(define-data-var available-ainomo uint u0)
(define-data-var borrowed-ainomo uint u0)
...
(define-public (set-available-ainomo (new-amount uint))
    (begin 
        (try! (check-is-owner))
        (ok (var-set available-ainomo new-amount))
    )
)

(define-read-only (get-available-ainomo)
  (var-get available-ainomo)
)

(define-read-only (get-borrowed-ainomo)
  (var-get borrowed-ainomo)
)
...
(define-public (claim-and-add-to-position-many (reward-cycles (list 50 uint)))
  (let 
    ...
    (try! (contract-call? .auto-ainomo add-to-position (get entitled-token total-claimed)))
    (ok claimed)
  )
)
...
(define-public (claim-and-add-to-position (reward-cycle uint))
  (let 
    ...
    (contract-call? .auto-ainomo add-to-position (get entitled-token claimed))
  )
)
...
(define-private (add-to-position-internal (dx uint))
    (let 
        (
          ...
          (ainomo (get dy positions))
          (atainomo-to-return (try! (contract-call? .auto-ainomo get-token-given-position ainomo)))
          (ainomo-available (get-available-ainomo))
          (ainomo-borrowed (get-borrowed-ainomo))      
        ...
        (asserts! (>= ainomo-available ainomo) ERR-AVAILABLE-AINOMO)
        ...
        (as-contract (try! (contract-call? .age000-governance-token mint-fixed ainomo sender)))
        (try! (contract-call? .auto-ainomo add-to-position ainomo))
        (var-set available-ainomo (- ainomo-available ainomo))
        (var-set borrowed-ainomo (+ ainomo-borrowed ainomo))
        (fold 
          update-balance-iter 
...
(define-public (claim-and-stake (reward-cycle uint))
  (let 
    (      
      ...
      (ainomo-claimed (if (is-eq (as-contract (get-ainomo-user-id)) u0) { to-return: u0, entitled-token: u0 } (as-contract (try! (claim-ainomo-staking-reward reward-cycle)))))
      (principal-to-stake (get to-return claimed))
      (ainomo-to-distribute (/ (get entitled-token claimed) u2))
      (ainomo-to-stake (- (+ (get entitled-token claimed) (get to-return ainomo-claimed) (get entitled-token ainomo-claimed)) ainomo-to-distribute))       
      (current-cycle (unwrap! (get-reward-cycle block-height) ERR-STAKING-NOT-AVAILABLE))      
    ...
    (asserts! (> ainomo-to-distribute bounty) ERR-INSUFFICIENT-BALANCE)
    
    (and (> principal-to-stake u0) (> cycles-to-stake u0) (as-contract (try! (stake-tokens principal-to-stake cycles-to-stake))))
    (and (> ainomo-to-stake u0) (> cycles-to-stake u0) (as-contract (try! (stake-ainomo-tokens ainomo-to-stake cycles-to-stake))))
    (as-contract (try! (contract-call? .auto-ainomo add-to-position (- ainomo-to-distribute bounty))))
    (and (> bounty u0) (as-contract (try! (contract-call? .age000-governance-token transfer-fixed bounty tx-sender sender none))))
    (map-set distributable-per-cycle reward-cycle (try! (contract-call? .auto-ainomo get-token-given-position (- ainomo-to-distribute bounty))))
    (ok true)
  )
)
...
(define-private (distribute-iter (recipient principal) (prior (response { cycle: uint, atainomo: uint, balance: uint, sum: uint } uint)))
  (let 
    (
      ...
      (shares (div-down (mul-down atainomo (get-user-balance-per-cycle-or-default recipient cycle)) balance))
    )
    (if (get-user-distributed-per-cycle-or-default recipient cycle)
      ...
      (begin 
        (and (> shares u0) (as-contract (try! (contract-call? .auto-ainomo transfer-fixed shares tx-sender recipient none))))
...
(define-public (distribute (cycle uint) (batch uint) (recipients (list 200 principal)))
    ...
    (let
      (
        (output (try! (fold distribute-iter recipients (ok { cycle: cycle, atainomo:  (get-distributable-per-cycle-or-default cycle), balance:  (get-total-balance-per-cycle-or-default cycle), sum: u0 }))))
      )
  ...
  (begin
    ...
    (let 
      (
        ...
        (ainomo-balance (mul-down percent (unwrap! (contract-call? .age000-governance-token get-balance-fixed (as-contract tx-sender)) ERR-GET-BALANCE-FIXED-FAIL)))
        (pool-reduced (as-contract (try! (contract-call? .fixed-weight-pool-v1-01 reduce-position .token-wstx .age000-governance-token u50000000 u50000000 .fwp-wstx-ainomo-50-50-v1-01 percent))))
        (stx-reduced (get dx pool-reduced))
        (ainomo-reduced (get dy pool-reduced))
        ...
        (ainomo-available (+ ainomo-reduced ainomo-balance))
        (ainomo-to-sell-uncapped (if (is-eq stx-to-buy u0) u0 (try! (contract-call? .fixed-weight-pool-v1-01 get-y-in-given-wstx-out .age000-governance-token u50000000 stx-to-buy))))      
        (ainomo-to-sell (if (<= ainomo-to-sell-uncapped ainomo-available) ainomo-to-sell-uncapped ainomo-available))
        (stx-bought (if (is-eq ainomo-to-sell u0) u0 (get dx (as-contract (try! (contract-call? .fixed-weight-pool-v1-01 swap-y-for-wstx .age000-governance-token u50000000 ainomo-to-sell none))))))
        (stx-available (+ stx-reduced stx-bought))
        (stx-to-return (if (<= stx-available stx-promised) stx-available stx-promised))
        (ainomo-residual (- ainomo-available ainomo-to-sell))
      ...
      (and (> ainomo-residual u0) (as-contract (try! (contract-call? .age000-governance-token transfer-fixed ainomo-residual tx-sender (var-get contract-owner) none))))
      (print { object: "pool", action: "position-reduced", data: { dx: stx-reduced, dy: ainomo-reduced } })
      (ok { stx-to-return: stx-to-return, stx-residual: stx-residual, ainomo-residual: ainomo-residual })
    )
  )
)
