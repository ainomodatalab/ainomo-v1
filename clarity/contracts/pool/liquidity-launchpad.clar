...
(define-public (create-pool
	...
	(let 
		(
			(launch-id (try! (contract-call? .ainomo-launchpad-v1-3 create-pool launch-token-trait payment-token-trait (merge offering { launch-owner: (as-contract tx-sender) }))))
		)
		(try! (contract-call? .ainomo-vault-v1-1 set-approved-token (contract-of launch-token-trait) true))
		(try! (contract-call? .ainomo-vault-v1-1 set-approved-token (contract-of payment-token-trait) true))
...
(define-public (add-to-position (launch-id uint) (tickets uint) (launch-token-trait <ft-trait>))
	(let
		...
		(as-contract (try! (contract-call? .ainomo-launchpad-v1-3 add-to-position launch-id tickets launch-token-trait)))
...
(define-read-only (get-launch (launch-id uint))
	(unwrap-panic (contract-call? .ainomo-launchpad-v1-3 get-launch launch-id))
)
...
(define-public (claim (launch-id uint) (input (list 200 principal)) (launch-token-trait <ft-trait>) (payment-token-trait <ft-trait>))
	(let 
		...
		(try! (contract-call? .ainomo-launchpad-v1-3 claim launch-id input launch-token-trait payment-token-trait))
