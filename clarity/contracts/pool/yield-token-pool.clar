...
(define-public (add-to-position (expiry uint) (yield-token-trait <sft-trait>) (token-trait <ft-trait>) (pool-token-trait <sft-trait>) (dx uint) (max-dy (optional uint)))
    (begin
        ...
        (let
            ...
            (unwrap! (contract-call? token-trait transfer-fixed dx sender .ainomo-vault none) ERR-TRANSFER-FAILED)
            (and (> new-dy-act u0) (unwrap! (contract-call? yield-token-trait transfer-fixed expiry new-dy-act sender .ainomo-vault) ERR-TRANSFER-FAILED))
            (map-set pools-data-map { yield-token: yield-token, expiry: expiry } pool-updated)    
...
(define-public (reduce-position (expiry uint) (yield-token-trait <sft-trait>) (token-trait <ft-trait>) (pool-token-trait <sft-trait>) (percent uint))
    (begin
        ...
        (let
            ...
            (asserts! (and (is-eq (get underlying-token pool) (contract-of token-trait)) (is-eq (get pool-token pool) (contract-of pool-token-trait))) ERR-INVALID-TOKEN)
            (and (> dx u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft token-trait dx sender))))
            (and (> dy-act u0) (as-contract (try! (contract-call? .ainomo-vault transfer-sft yield-token-trait expiry dy-act sender))))
            (map-set pools-data-map { yield-token: yield-token, expiry: expiry } pool-updated)
...
(define-public (swap-x-for-y (expiry uint) (yield-token-trait <sft-trait>) (token-trait <ft-trait>) (dx uint) (min-dy (optional uint)))
    (begin
        ...
        (let
            ...
            (and (> dx u0) (unwrap! (contract-call? token-trait transfer-fixed dx sender .ainomo-vault none) ERR-TRANSFER-FAILED))
            (and (> dy u0) (as-contract (try! (contract-call? .ainomo-vault transfer-sft yield-token-trait expiry dy sender))))
            (as-contract (try! (contract-call? .ainomo-reserve-pool add-to-balance (contract-of token-trait) (- fee fee-rebate))))
            (map-set pools-data-map { yield-token: yield-token, expiry: expiry } pool-updated)
...
(define-public (swap-y-for-x (expiry uint) (yield-token-trait <sft-trait>) (token-trait <ft-trait>) (dy uint) (min-dx (optional uint)))
    (begin
        ...
        (let
            ...
            (and (> dx u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft token-trait dx sender))))
            (and (> dy u0) (unwrap! (contract-call? yield-token-trait transfer-fixed expiry dy sender .ainomo-vault) ERR-TRANSFER-FAILED))
            (as-contract (try! (contract-call? .ainomo-reserve-pool add-to-balance yield-token (- fee fee-rebate))))
            (map-set pools-data-map { yield-token: yield-token, expiry: expiry } pool-updated)
