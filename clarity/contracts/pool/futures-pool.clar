...
;; futures pool
;; DOES NOT WORK DUE TO ainomo-reserve-pool requiring 1 principal <> user-id
...
(define-private (get-user-id (token principal))
  (default-to u0 (contract-call? .ainomo-reserve-pool get-user-id token tx-sender))
)
(define-private (get-reward-cycle (token principal) (stack-height uint))
  (contract-call? .ainomo-reserve-pool get-reward-cycle token stack-height)
)
(define-private (stake-tokens (token-trait <ft-trait>) (amount-tokens uint) (lock-period uint))
  (contract-call? .ainomo-reserve-pool stake-tokens token-trait amount-tokens lock-period)
)
(define-private (get-first-stacks-block-in-reward-cycle (token principal) (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool get-first-stacks-block-in-reward-cycle token reward-cycle)
)
(define-private (claim-staking-reward (token-trait <ft-trait>) (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool claim-staking-reward token-trait reward-cycle)
)
...
(define-public (create-pool (staked-token principal) (reward-cycles (list 32 uint)) (yield-token principal)) 
    (let
        ...
        (try! (contract-call? .ainomo-vault add-approved-token staked-token))
        (try! (contract-call? .ainomo-vault add-approved-token yield-token))
...
(define-public (reduce-position (staked-token-trait <ft-trait>) (start-cycle uint) (yield-token-trait <sft-trait>) (percent uint))
  (begin
    ...
    (let
        ...
        (asserts! (is-eq (get pool-token pool) (contract-of yield-token-trait)) ERR-INVALID-TOKEN)
        (asserts! (> block-height (+ (get-first-stacks-block-in-reward-cycle staked-token (+ start-cycle u31)) (contract-call? .ainomo-reserve-pool get-reward-cycle-length))) ERR-STAKING-IN-PROGRESS)
        
        ;; the first call claims rewards/stakes
        (and 
          (not (get claimed pool))
          (> (as-contract (get-user-id .age000-governance-token)) u0) 
          (is-ok (as-contract (fold check-err (map claim-ainomo-staking-reward reward-cycles) (ok { entitled-token: u0, to-return: u0 }))))
        )
        (and 
          (not (get claimed pool))          
