...
(define-private (add-approved-token-to-vault (token principal))
    (contract-call? .ainomo-vault add-approved-token token)
)
...
(define-public (add-to-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint) (pool-token-trait <ft-trait>) (dx uint) (max-dy (optional uint)))
    (begin
        ...
        (let
            ...
            (unwrap! (contract-call? token-x-trait transfer-fixed dx sender .ainomo-vault none) ERR-TRANSFER-FAILED)
            (unwrap! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault none) ERR-TRANSFER-FAILED)
...
(define-public (reduce-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint) (pool-token-trait <ft-trait>) (percent uint))
    (begin
        ...
        (let
            ...
            (as-contract (try! (contract-call? .ainomo-vault transfer-ft-two token-x-trait dx token-y-trait dy sender)))
...
;; @desc swap-ainomo-for-y
;; @params token-y-trait; ft-trait
;; @params weight-y
;; @params dx 
;; @params min-dy 
;; @returns (ok (tuple))
(define-public (swap-ainomo-for-y (token-y-trait <ft-trait>) (weight-y uint) (dx uint) (min-dy (optional uint)))    
    (begin
        ...
        (let
            (
                ...
                (dy (try! (get-y-given-ainomo token-y weight-y dx-net-fees)))                
            ...
            (unwrap! (contract-call? .age000-governance-token transfer-fixed dx sender .ainomo-vault none) ERR-TRANSFER-FAILED)
            (and (> dy u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft token-y-trait dy sender))))
            (as-contract (try! (contract-call? .ainomo-reserve-pool add-to-balance .age000-governance-token (- fee fee-rebate))))
...
;; @desc swap-y-for-ainomo 
;; @params token-y-trait; ft-
;; @params weight-y 
;; @params dy
;; @params dx
;; @returns (response tuple)
(define-public (swap-y-for-ainomo (token-y-trait <ft-trait>) (weight-y uint) (dy uint) (min-dx (optional uint)))
    (begin
        (try! (check-pool-status .age000-governance-token (contract-of token-y-trait) (- ONE_8 weight-y) weight-y))
        (asserts! (> dy u0) ERR-INVALID-LIQUIDITY)
        (let
            (
                ...
                (dx (try! (get-ainomo-given-y token-y weight-y dy-net-fees)))
            ...
            (and (> dx u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft .age000-governance-token dx sender))))
            (unwrap! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault none) ERR-TRANSFER-FAILED)
    ...
    (ok 
        {
            ...
            dy: 
                (if (is-eq (contract-of token-x-trait) .age000-governance-token)
                    (get dy (try! (swap-ainomo-for-y token-y-trait weight-y dx min-dy)))
                    (if (is-eq (contract-of token-y-trait) .age000-governance-token)
                        (get dx (try! (swap-y-for-ainomo token-x-trait weight-x dx min-dy)))
                        (get dy (try! (swap-ainomo-for-y token-y-trait weight-y (get dx (try! (swap-y-for-ainomo token-x-trait weight-x dx none))) min-dy)))
                    )
                )
        }
    )
    ...
    (ok 
        {
            dx:
                (if (is-eq (contract-of token-x-trait) .age000-governance-token)
                    (get dx (try! (swap-y-for-ainomo token-y-trait weight-y dy min-dx)))
                    (if (is-eq (contract-of token-y-trait) .age000-governance-token)
                        (get dy (try! (swap-ainomo-for-y token-x-trait weight-x dy min-dx)))
                        (get dy (try! (swap-ainomo-for-y token-x-trait weight-x (get dx (try! (swap-y-for-ainomo token-y-trait weight-y dy none))) min-dx)))
                    )
                ),
            dy: dy
        }
...
;; @desc get-y-given-ainomo
;; @params token-y-trait; ft-trait 
;; @params weight-y 
;; @params dx 
;; @returns (respons uint uint)
(define-read-only (get-y-given-ainomo (token-y principal) (weight-y uint) (dx uint))
    (let 
        (
            (weight-x (- ONE_8 weight-y))
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y, weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL))
...
;; @desc get-ainomo-given-y
;; @params token-y-trait; ft-trait 
;; @params weight-y 
;; @params dy
;; @returns (respons uint uint)
(define-read-only (get-ainomo-given-y (token-y principal) (weight-y uint) (dy uint)) 
    (let 
        (
            (weight-x (- ONE_8 weight-y))
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y, weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL))
...
(define-read-only (get-y-given-x (token-x principal) (token-y principal) (weight-x uint) (weight-y uint) (dx uint))
    (if (is-eq token-x .age000-governance-token)
        (get-y-given-ainomo token-y weight-y dx)
        (if (is-eq token-y .age000-governance-token)
            (get-ainomo-given-y token-x weight-x dx)
            (get-y-given-ainomo token-y weight-y (try! (get-ainomo-given-y token-x weight-x dx)))
        )
    )
)
...
(define-read-only (get-x-given-y (token-x principal) (token-y principal) (weight-x uint) (weight-y uint) (dy uint)) 
    (if (is-eq token-x .age000-governance-token)
        (get-ainomo-given-y token-y weight-y dy)
        (if (is-eq token-y .age000-governance-token)
            (get-y-given-ainomo token-x weight-x dy)
            (get-y-given-ainomo token-x weight-x (try! (get-ainomo-given-y token-y weight-y dy)))
        )
    )
)

(define-read-only (get-y-in-given-ainomo-out (token-y principal) (weight-y uint) (dx uint))
    (let 
        (
            (weight-x (- ONE_8 weight-y))
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y, weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL))
...
(define-read-only (get-ainomo-in-given-y-out (token-y principal) (weight-y uint) (dy uint)) 
    (let 
        (
            (weight-x (- ONE_8 weight-y))
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y, weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL))
...
(define-read-only (get-y-in-given-x-out (token-x principal) (token-y principal) (weight-x uint) (weight-y uint) (dx uint))
    (if (is-eq token-x .age000-governance-token)
        (get-y-in-given-ainomo-out token-y weight-y dx)
        (if (is-eq token-y .age000-governance-token)
            (get-ainomo-in-given-y-out token-x weight-x dx)
            (get-y-in-given-ainomo-out token-y weight-y (try! (get-ainomo-in-given-y-out token-x weight-x dx)))
        )
    )
)
...
(define-read-only (get-x-in-given-y-out (token-x principal) (token-y principal) (weight-x uint) (weight-y uint) (dy uint)) 
    (if (is-eq token-x .age000-governance-token)
        (get-ainomo-in-given-y-out token-y weight-y dy)
        (if (is-eq token-y .age000-governance-token)
            (get-y-in-given-ainomo-out token-x weight-x dy)
            (get-y-in-given-ainomo-out token-x weight-x (try! (get-ainomo-in-given-y-out token-y weight-y dy)))
        )
    )
)
