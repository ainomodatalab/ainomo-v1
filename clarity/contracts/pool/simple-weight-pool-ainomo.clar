...
;; @params min-dy 
;; @returns (ok (tuple))
(define-public (swap-ainomo-for-y (token-y-trait <ft-trait>) (dx uint) (min-dy (optional uint)))    
    (begin
        ...
        (let
            (
                ...
                (dy (try! (get-y-given-ainomo token-y dx-net-fees)))                
...
(define-public (swap-y-for-ainomo (token-y-trait <ft-trait>) (dy uint) (min-dx (optional uint)))
    (begin
        (try! (check-pool-status .age000-governance-token (contract-of token-y-trait)))
        (asserts! (> dy u0) ERR-INVALID-LIQUIDITY)
        (let
            (
                ...
                (dx (try! (get-ainomo-given-y token-y dy-net-fees)))
    ...
    (ok 
        {
            ...
            dy: 
                (if (is-eq (contract-of token-x-trait) .age000-governance-token)
                    (get dy (try! (swap-ainomo-for-y token-y-trait dx min-dy)))
                    (if (is-eq (contract-of token-y-trait) .age000-governance-token)
                        (get dx (try! (swap-y-for-ainomo token-x-trait dx min-dy)))
                        (get dy (try! (swap-ainomo-for-y token-y-trait (get dx (try! (swap-y-for-ainomo token-x-trait dx none))) min-dy)))
                    )
                )
        }
    )
    ...
    (ok 
        {
            dx:
                (if (is-eq (contract-of token-x-trait) .age000-governance-token)
                    (get dx (try! (swap-y-for-ainomo token-y-trait dy min-dx)))
                    (if (is-eq (contract-of token-y-trait) .age000-governance-token)
                        (get dy (try! (swap-ainomo-for-y token-x-trait dy min-dx)))
                        (get dy (try! (swap-ainomo-for-y token-x-trait (get dx (try! (swap-y-for-ainomo token-y-trait dy none))) min-dx)))
                    )
                ),
            dy: dy
        }
...
(define-read-only (get-y-given-ainomo (token-y principal) (dx uint))
    (let 
        (
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y }) ERR-INVALID-POOL))
        )
...
;; @params dy
;; @returns (respons uint uint)
(define-read-only (get-ainomo-given-y (token-y principal) (dy uint)) 
    (let 
        (
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y }) ERR-INVALID-POOL))
        )
...
(define-read-only (get-y-given-x (token-x principal) (token-y principal) (dx uint))
    (if (is-eq token-x .age000-governance-token)
        (get-y-given-ainomo token-y dx)
        (if (is-eq token-y .age000-governance-token)
            (get-ainomo-given-y token-x dx)
            (get-y-given-ainomo token-y (try! (get-ainomo-given-y token-x dx)))
        )
    )
)
...
(define-read-only (get-x-given-y (token-x principal) (token-y principal) (dy uint)) 
    (if (is-eq token-x .age000-governance-token)
        (get-ainomo-given-y token-y dy)
        (if (is-eq token-y .age000-governance-token)
            (get-y-given-ainomo token-x dy)
            (get-y-given-ainomo token-x (try! (get-ainomo-given-y token-y dy)))
        )
    )
)

(define-read-only (get-y-in-given-ainomo-out (token-y principal) (dx uint))
    (let 
        (
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y }) ERR-INVALID-POOL))
        )
...
(define-read-only (get-ainomo-in-given-y-out (token-y principal) (dy uint)) 
    (let 
        (
            (pool (unwrap! (map-get? pools-data-map { token-x: .age000-governance-token, token-y: token-y }) ERR-INVALID-POOL))
        )
...
(define-read-only (get-y-in-given-x-out (token-x principal) (token-y principal) (dx uint))
    (if (is-eq token-x .age000-governance-token)
        (get-y-in-given-ainomo-out token-y dx)
        (if (is-eq token-y .age000-governance-token)
            (get-ainomo-in-given-y-out token-x dx)
            (get-y-in-given-ainomo-out token-y (try! (get-ainomo-in-given-y-out token-x dx)))
        )
    )
)
...
(define-read-only (get-x-in-given-y-out (token-x principal) (token-y principal) (dy uint)) 
    (if (is-eq token-x .age000-governance-token)
        (get-ainomo-in-given-y-out token-y dy)
        (if (is-eq token-y .age000-governance-token)
            (get-y-in-given-ainomo-out token-x dy)
            (get-y-in-given-ainomo-out token-x (try! (get-ainomo-in-given-y-out token-y dy)))
        )
    )
)
