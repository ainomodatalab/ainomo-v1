...
(define-public (swap-wstx-for-y (token-y-trait <ft-trait>) (dx uint) (min-dy (optional uint)))    
    (begin
        ...
        (let
            ...
            (unwrap! (contract-call? .token-wstx transfer-fixed dx sender .ainomo-vault none) ERR-TRANSFER-FAILED)
            (and (> dy u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft token-y-trait dy sender))))
            (as-contract (try! (contract-call? .ainomo-reserve-pool add-to-balance .token-wstx (- fee fee-rebate))))
...
(define-public (swap-y-for-wstx (token-y-trait <ft-trait>) (dy uint) (min-dx (optional uint)))
    (begin
        ...
        (let
            ...
            (and (> dx u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft .token-wstx dx sender))))
            (unwrap! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault none) ERR-TRANSFER-FAILED)
