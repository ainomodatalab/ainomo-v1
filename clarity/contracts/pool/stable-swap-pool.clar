...
(define-public (swap-wxusd-for-y (token-y-trait <ft-trait>) (factor uint) (dx uint) (min-dy (optional uint)))    
    (let
        ...
        (unwrap! (contract-call? .token-wxusd transfer-fixed dx sender .ainomo-vault none) ERR-TRANSFER-FAILED)
        (and (> dy u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft token-y-trait dy sender))))
        (as-contract (try! (contract-call? .ainomo-reserve-pool add-to-balance .token-wxusd (- fee fee-rebate))))
...
(define-public (swap-y-for-wxusd (token-y-trait <ft-trait>) (factor uint) (dy uint) (min-dx (optional uint)))
    (let
        ...
        (unwrap! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault none) ERR-TRANSFER-FAILED)
        (and (> dx u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft .token-wxusd dx sender))))            
