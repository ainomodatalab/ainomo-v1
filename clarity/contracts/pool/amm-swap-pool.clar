...
(define-public (add-to-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (max-dy (optional uint)))
    (let
        ...
        (try! (contract-call? token-x-trait transfer-fixed dx sender .ainomo-vault none))
        (try! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault none))
...
(define-public (swap-y-for-x (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dy uint) (min-dx (optional uint)))
    (let
        ...
        (try! (contract-call? token-y-trait transfer-fixed dy sender .ainomo-vault none))
        (and (> dx u0) (as-contract (try! (contract-call? .ainomo-vault transfer-ft token-x-trait dx sender))))            
