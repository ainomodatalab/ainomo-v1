...
(define-fungible-token lbp-ainomo-wstx-80-20)
...
(define-read-only (get-total-supply)
  (ok (ft-get-supply lbp-ainomo-wstx-80-20))
)

;; @desc get-name
;; @returns (response string-utf8)
(define-read-only (get-name)
  (ok "lbp-ainomo-wstx-80-20")
)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance lbp-ainomo-wstx-80-20 account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? lbp-ainomo-wstx-80-20 amount sender recipient))
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? lbp-ainomo-wstx-80-20 amount recipient)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved tx-sender))
    (ft-burn? lbp-ainomo-wstx-80-20 amount sender)
  )
)
...
(define-read-only (get-total-supply-fixed)
  (ok (decimals-to-fixed (ft-get-supply lbp-ainomo-wstx-80-20)))
)
...
(define-read-only (get-balance-fixed (account principal))
  (ok (decimals-to-fixed (ft-get-balance lbp-ainomo-wstx-80-20 account)))
)
