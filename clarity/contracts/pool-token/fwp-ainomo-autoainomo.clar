...
(define-fungible-token fwp-ainomo-autoainomo)
...
(define-read-only (get-total-supply)
  (ok (ft-get-supply fwp-ainomo-autoainomo))
)

;; @desc get-name
;; @returns (response string-utf8)
(define-read-only (get-name)
  (ok "fwp-ainomo-autoainomo")
)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance fwp-ainomo-autoainomo account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? fwp-ainomo-autoainomo amount sender recipient))
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? fwp-ainomo-autoainomo amount recipient)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved tx-sender))
    (ft-burn? fwp-ainomo-autoainomo amount sender)
  )
)
...
(define-read-only (get-total-supply-fixed)
  (ok (decimals-to-fixed (ft-get-supply fwp-ainomo-autoainomo)))
)
...
(define-read-only (get-balance-fixed (account principal))
  (ok (decimals-to-fixed (ft-get-balance fwp-ainomo-autoainomo account)))
)
...
(map-set approved-contracts .simple-weight-pool-ainomo true)
