...
(define-fungible-token fwp-wstx-ainomo-50-50-v1-01)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "STX-AINOMO Pool Token Weight 50/50")
(define-data-var token-symbol (string-ascii 32) "STX-AINOMO-50-50")
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? fwp-wstx-ainomo-50-50-v1-01 amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance fwp-wstx-ainomo-50-50-v1-01 who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply fwp-wstx-ainomo-50-50-v1-01))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? fwp-wstx-ainomo-50-50-v1-01 amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? fwp-wstx-ainomo-50-50-v1-01 amount sender)
	)
)
