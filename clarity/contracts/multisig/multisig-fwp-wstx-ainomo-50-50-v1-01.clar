...
(define-read-only (is-token-accepted (token principal))
  (is-eq token .fwp-wstx-ainomo-50-50-v1-01)
)
...
(define-public (propose (start-block-height uint) (title (string-utf8 256)) (url (string-utf8 256)) (new-fee-rate-x uint) (new-fee-rate-y uint))
  (let 
    (
      (proposer-balance (unwrap-panic (contract-call? .fwp-wstx-ainomo-50-50-v1-01 get-balance tx-sender)))
      (total-supply (unwrap-panic (contract-call? .fwp-wstx-ainomo-50-50-v1-01 get-total-supply)))
