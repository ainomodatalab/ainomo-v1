...
(define-public (propose (start-block-height uint) (title (string-utf8 256)) (url (string-utf8 256)) (new-fee-rate-x uint) (new-fee-rate-y uint))
  (let 
    (
      (proposer-balance (unwrap-panic (contract-call? .fwp-ainomo-wmia get-balance tx-sender)))
      (total-supply (unwrap-panic (contract-call? .fwp-ainomo-wmia get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .simple-weight-pool-ainomo set-fee-rate-x .age000-governance-token .token-wmia new-fee-rate-x)))
    (as-contract (try! (contract-call? .simple-weight-pool-ainomo set-fee-rate-y .age000-governance-token .token-wmia new-fee-rate-y)))
    (ok true)
  )
)
