...
(define-public (propose
  ...
  (let (
    (proposer-balance (unwrap-panic (contract-call? .ytp-ainomo-v1 get-balance-fixed expiry tx-sender)))
    (total-supply (unwrap-panic (contract-call? .ytp-ainomo-v1 get-total-supply-fixed expiry)))
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    (as-contract (try! (contract-call? .yield-token-pool set-fee-rate-token expiry .yield-ainomo new-fee-rate-token)))
    (as-contract (try! (contract-call? .yield-token-pool set-fee-rate-yield-token expiry .yield-ainomo new-fee-rate-yield-token)))
