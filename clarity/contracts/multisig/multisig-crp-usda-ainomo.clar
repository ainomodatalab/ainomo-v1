...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-usda) (is-eq token .key-usda-ainomo))
)
...
(define-public (propose  
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-usda-ainomo get-balance-fixed expiry tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-usda-ainomo get-total-supply-fixed expiry)))
