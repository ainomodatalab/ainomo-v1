...
(define-read-only (is-token-accepted (token principal))
    (is-eq token .lbp-ainomo-wstx-80-20)
)
...
(define-public (propose
  ...
  (let (
    (proposer-balance (unwrap-panic (contract-call? .lbp-ainomo-wstx-80-20 get-balance-fixed tx-sender)))
    (total-supply (unwrap-panic (contract-call? .lbp-ainomo-wstx-80-20 get-total-supply-fixed)))
