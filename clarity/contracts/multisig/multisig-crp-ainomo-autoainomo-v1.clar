...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-ainomo-v1) (is-eq token .key-ainomo-autoainomo-v1))
)
...
(define-public (propose  
  ...
  (let 
    (
      (proposer-yield-balance (unwrap-panic (contract-call? .yield-ainomo-v1 get-balance-fixed expiry tx-sender)))
      (proposer-key-balance (unwrap-panic (contract-call? .key-ainomo-autoainomo-v1 get-balance-fixed expiry tx-sender)))
      (proposer-balance (+ proposer-yield-balance proposer-key-balance))
      (total-yield-supply (unwrap-panic (contract-call? .yield-ainomo-v1 get-total-supply-fixed expiry)))
      (total-key-supply (unwrap-panic (contract-call? .key-ainomo-autoainomo-v1 get-total-supply-fixed expiry)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-x .age000-governance-token .auto-ainomo expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-y .age000-governance-token .auto-ainomo expiry new-fee-rate-y)))
