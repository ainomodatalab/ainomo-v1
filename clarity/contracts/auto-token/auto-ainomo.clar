...
(define-fungible-token auto-ainomo)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "Auto AINOMO")
(define-data-var token-symbol (string-ascii 10) "auto-ainomo")
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? auto-ainomo amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance auto-ainomo who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply auto-ainomo))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? auto-ainomo amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? auto-ainomo amount sender)
	)
)
  ...
  (let
    ...
    (let
      ...
      (var-set total-supply new-total-supply)
	    (try! (ft-mint? auto-ainomo (fixed-to-decimals new-supply) sender))
...
;; claims ainomo for the reward-cycles and mint auto-ainomo
...
(define-public (reduce-position (percent uint))
  (let 
    ...
    ;; burn pool token
    (var-set total-supply new-total-supply)
	  (try! (ft-burn? auto-ainomo (fixed-to-decimals reduce-supply) sender))
