...
(define-fungible-token auto-fwp-ainomo-autoainomo-x-v1-01)
...
(define-data-var token-symbol (string-ascii 32) "auto-fwp-ainomo-autoainomo-x-v1-01")
...
(define-read-only (get-overall-balance (who principal))
	(ok (ft-get-balance auto-fwp-ainomo-autoainomo-x-v1-01 who))
)
...
(define-read-only (get-overall-supply)
	(ok (ft-get-supply auto-fwp-ainomo-autoainomo-x-v1-01))
)
...
(define-public (transfer (token-id uint) (amount uint) (sender principal) (recipient principal))
    ...
    (asserts! (var-get transferrable) ERR-TRANSFER-FAILED)
		...
		(try! (ft-transfer? auto-fwp-ainomo-autoainomo-x-v1-01 amount sender recipient))
...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
	(begin
		...
		(try! (ft-mint? auto-fwp-ainomo-autoainomo-x-v1-01 amount recipient))
		(try! (set-balance token-id (+ (get-balance-or-default token-id recipient) amount) recipient))
...
(define-public (burn (token-id uint) (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner)) (is-ok (check-is-self))) ERR-NOT-AUTHORIZED)
		(try! (ft-burn? auto-fwp-ainomo-autoainomo-x-v1-01 amount sender))
...
(define-read-only (get-overall-supply-fixed)
	(ok (decimals-to-fixed (ft-get-supply auto-fwp-ainomo-autoainomo-x-v1-01)))
)
...
(define-read-only (get-overall-balance-fixed (who principal))
	(ok (decimals-to-fixed (ft-get-balance auto-fwp-ainomo-autoainomo-x-v1-01 who)))
)
...
(define-public (add-to-position (tranche uint) (dx uint))
    (let 
        ...
        (as-contract (try! (contract-call? .simple-weight-pool-ainomo add-to-position .age000-governance-token .auto-ainomo .fwp-ainomo-autoainomo (- dx (mul-down (var-get add-multiplier) ainomo-to-atainomo)) (some atainomo-amount))))
