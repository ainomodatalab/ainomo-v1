...
(define-fungible-token auto-ainomo-v2)

(define-data-var contract-owner principal tx-sender)
(define-data-var token-name (string-ascii 32) "Auto AINOMO")
(define-data-var token-symbol (string-ascii 32) "auto-ainomo-v2")
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance auto-ainomo-v2 who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply auto-ainomo-v2))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? auto-ainomo-v2 amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? auto-ainomo-v2 amount sender)
	)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? auto-ainomo-v2 amount sender recipient))
...
;; -- autoAINOMO creation/staking/redemption
...
(define-data-var bounty-in-fixed uint u100000000) ;; 1 AINOMO
...
;; @desc get the intrinsic value of auto-ainomo-v2
;; @desc intrinsic = next capital base of the vault / total supply of auto-ainomo-v2
...
;; @desc transfers dx to vault, stake them for 32 cycles and mints auto-ainomo-v2, the number of which is determined as % of total supply / next base
  ...
  (let
    ...
    (let
      ...
      (var-set total-supply new-total-supply)
	    (try! (ft-mint? auto-ainomo-v2 (fixed-to-decimals new-supply) sender))
...
;; claims ainomo for the reward-cycles and mint auto-ainomo-v2
...
(define-public (reduce-position (percent uint))
  (let 
    ...
    ;; burn pool token
    (var-set total-supply new-total-supply)
	  (try! (ft-burn? auto-ainomo-v2 (fixed-to-decimals reduce-supply) sender))
...
(define-private (get-staking-reward (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool get-staking-reward .age000-governance-token (get-user-id) reward-cycle)
)
(define-private (get-staker-at-cycle (reward-cycle uint))
  (contract-call? .ainomo-reserve-pool get-staker-at-cycle-or-default .age000-governance-token reward-cycle (get-user-id))
)
