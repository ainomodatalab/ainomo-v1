...
(define-fungible-token auto-fwp-wstx-ainomo-120)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "Auto STX / AINOMO Pool 120")
(define-data-var token-symbol (string-ascii 32) "auto-fwp-wstx-ainomo-120")
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? auto-fwp-wstx-ainomo-120 amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance auto-fwp-wstx-ainomo-120 who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply auto-fwp-wstx-ainomo-120))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? auto-fwp-wstx-ainomo-120 amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? auto-fwp-wstx-ainomo-120 amount sender)
	)
)
  ...
  (let
    ...
    (let
      ...
      (var-set total-supply new-total-supply)
			(try! (ft-mint? auto-fwp-wstx-ainomo-120 (fixed-to-decimals (get token new-supply)) sender))
...
(define-public (reduce-position (percent uint))
  (let 
    ...
    ;; burn pool token
    (var-set total-supply new-total-supply)
		(try! (ft-burn? auto-fwp-wstx-ainomo-120 (fixed-to-decimals reduce-supply) sender))
...
;; (contract-call? .ainomo-vault add-approved-token .auto-fwp-wstx-ainomo-120)
