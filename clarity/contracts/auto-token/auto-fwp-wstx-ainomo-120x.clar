...
(define-fungible-token auto-fwp-wstx-ainomo-120x)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "Auto STX / AINOMO Pool 120x")
(define-data-var token-symbol (string-ascii 32) "auto-fwp-wstx-ainomo-120x")
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))	
    (begin
        ...
        (try! (ft-transfer? auto-fwp-wstx-ainomo-120x amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance auto-fwp-wstx-ainomo-120x who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply auto-fwp-wstx-ainomo-120x))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? auto-fwp-wstx-ainomo-120x amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? auto-fwp-wstx-ainomo-120x amount sender)
	)
)
...
(define-public (add-to-position (dx uint))
    (let 
        (
          ...
          (vault (try! (contract-call? .auto-fwp-wstx-ainomo-120 get-token-given-position (get token pool))))
          (ainomo-required (+ (get dy pool) (get rewards vault)))
        ...
        (as-contract (try! (contract-call? .fixed-weight-pool-v1-01 add-to-position .token-wstx .age000-governance-token u50000000 u50000000 .fwp-wstx-ainomo-50-50-v1-01 dx (some (get dy pool)))))
        (as-contract (try! (contract-call? .auto-fwp-wstx-ainomo-120 add-to-position (get token pool))))
        ...
        (map-set borrowed-ainomo sender (+ ainomo-borrowed ainomo-required))
		    (try! (ft-mint? auto-fwp-wstx-ainomo-120x (fixed-to-decimals (get token vault)) sender))
...
(define-public (reduce-position)
  (let 
    (
      ...
      (vault-reduced (as-contract (try! (contract-call? .auto-fwp-wstx-ainomo-120 reduce-position share))))
      (pool-reduced (as-contract (try! (contract-call? .fixed-weight-pool-v1-01 reduce-position .token-wstx .age000-governance-token u50000000 u50000000 .fwp-wstx-ainomo-50-50-v1-01 share))))
      (ainomo-returned (+ (get dy pool-reduced) (get rewards vault-reduced)))
      ...
      (stx-to-sell (if (is-eq ainomo-to-buy u0) u0 (try! (contract-call? .fixed-weight-pool-v1-01 get-wstx-in-given-y-out .age000-governance-token u50000000 ainomo-to-buy))))
      (ainomo-bought (if (is-eq stx-to-sell u0) u0 (get dy (as-contract (try! (contract-call? .fixed-weight-pool-v1-01 swap-wstx-for-y .age000-governance-token u50000000 stx-to-sell (some (- ainomo-borrowed ainomo-returned))))))))
	...
	(try! (ft-burn? auto-fwp-wstx-ainomo-120x (fixed-to-decimals supply) sender))
    (print { object: "pool", action: "position-reduced", data: supply })
    (ok { stx: stx-to-return, ainomo: ainomo-to-return })
  )
)
