...
(define-fungible-token lottery-ido-ainomo)
...
(define-read-only (get-total-supply)
  (ok (ft-get-supply lottery-ido-ainomo))
)

(define-read-only (get-name)
  (ok "lottery-ido-ainomo")
)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance lottery-ido-ainomo account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? lottery-ido-ainomo amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? lottery-ido-ainomo amount recipient)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved tx-sender))
    (ft-burn? lottery-ido-ainomo amount sender)
  )
)
...
(define-read-only (get-total-supply-fixed)
  (ok (decimals-to-fixed (ft-get-supply lottery-ido-ainomo)))
)

(define-read-only (get-balance-fixed (account principal))
  (ok (decimals-to-fixed (ft-get-balance lottery-ido-ainomo account)))
)
...
(map-set approved-contracts .ainomo-launchpad true)