...
(define-fungible-token lottery-t-ainomo)
...
(define-read-only (get-total-supply)
  (ok (ft-get-supply lottery-t-ainomo))
)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance lottery-t-ainomo account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? lottery-t-ainomo amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? lottery-t-ainomo amount recipient)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved tx-sender))
    (ft-burn? lottery-t-ainomo amount sender)
  )
)
...
(define-read-only (get-total-supply-fixed)
  (ok (decimals-to-fixed (ft-get-supply lottery-t-ainomo)))
)

(define-read-only (get-balance-fixed (account principal))
  (ok (decimals-to-fixed (ft-get-balance lottery-t-ainomo account)))
)
