...
import { SSPTestAgent2 } from './models/ainomo-tests-amm-swap-pool.ts';
import { USDAToken, WBTCToken, WXUSDToken } from './models/ainomo-tests-tokens.ts';
...
async function setup(chain: Chain, accounts: Map<string, Account>, _factor?: number) {
  ...
  let block = chain.mineBlock([
    Tx.contractCall("ainomo-vault", "add-approved-token", [
  ...
  block = chain.mineBlock([
    Tx.contractCall("ainomo-vault", "add-approved-contract", [
      types.principal(deployer.address + '.amm-swap-pool')
    ], deployer.address),
    Tx.contractCall("ainomo-reserve-pool", "add-approved-contract", [
      types.principal(deployer.address + '.amm-swap-pool')
    ], deployer.address),
...
Clarinet.test({
  name: "amm-swap-pool : factor = 0.0001e8",
  async fn(chain: Chain, accounts: Map<string, Account>) {
