...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        // non contract-owner calling mint-ainomo-many throws an error.
...
Clarinet.test({
  name: "autoainomo-apower-helper : mint and burn apower",
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
      ...
      // non contract-owner calling mint-ainomo-many throws an error.
      let result:any = await TokenTest.mintFixed(deployer, 10_000e8, deployer.address + '.auto-ainomo');
      result.expectOk();
      result = await TokenTest.addApprovedContract(deployer, deployer.address + '.autoainomo-apower-helper');
      result.expectOk();

      let block:any = chain.mineBlock(
        [
          Tx.contractCall(deployer.address + '.autoainomo-apower-helper', 'set-approved-contract',
            [
          ...
          Tx.contractCall(deployer.address + '.autoainomo-apower-helper', 'mint-and-burn-apower', 
            [
              types.uint(1),
              types.uint(1),
