import {
  ...
  extractParameters,
  determineApower,
} from "./models/ainomo-tests-launchpad-v1-2.ts";
import type {
  Chain,
  Account,
...
Clarinet.test({
  name: "ainomo-launchpad-v1-2 : only owner can create pool",
...
Clarinet.test({
  name: "ainomo-launchpad-v1-2 : only launch-owner/approved-address can add-to-positions",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "ainomo-launchpad-v1-2 : registration is allowed only once",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "ainomo-launchpad-v1-2 : attempt to register more than max fails",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "ainomo-launchpad-v1-2 : contract-owner can transfer all tokens in emergency",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
    ...
    block.receipts[2].events.expectSTXTransferEvent(
      ...
      deployer.address + ".ainomo-launchpad-v1-2",
      deployer.address
    );
...
Clarinet.test({
  name: "ainomo-launchpad-v1-2 : activation-threshold not reached allows orderly refund",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "ainomo-launchpad-v1-2 : example claim walk test",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
