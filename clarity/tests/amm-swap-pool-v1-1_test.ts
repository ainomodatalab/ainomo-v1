...
import { SSPTestAgent3 } from './models/ainomo-tests-amm-swap-pool.ts';
...
async function setup(chain: Chain, accounts: Map<string, Account>, _factor?: number) {
  ...
  let block = chain.mineBlock([
    Tx.contractCall("ainomo-vault-v1-1", "set-approved-token", [
  ...
  block = chain.mineBlock([
    Tx.contractCall("ainomo-vault-v1-1", "set-approved-contract", [
...
Clarinet.test({
  name: "amm-swap-pool-v1-1 : factor = 1e8",
  async fn(chain: Chain, accounts: Map<string, Account>) {
...
Clarinet.test({
  name: "amm-swap-pool-v1-1 : factor = 0.0001e8",
  async fn(chain: Chain, accounts: Map<string, Account>) {
...
Clarinet.test({
  name: "amm-swap-pool-v1-1 : factor = 0.5e8",
  async fn(chain: Chain, accounts: Map<string, Account>) {
