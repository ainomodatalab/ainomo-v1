...
class FWPTestAgent2 {
  ...
  getPoolDetails(tokenX: string, tokenY: string, weightX: number, weightY: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-pool-details", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  getOracleResilient(tokenX: string, tokenY: string, weightX: number, weightY: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-oracle-resilient", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  getOracleInstant(tokenX: string, tokenY: string, weightX: number, weightY: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-oracle-instant", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  setOracleEnabled(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-oracle-enabled", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY)
  ...
  setOracleAverage(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, average: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-oracle-average", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  createPool(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, pooltoken: string, multisig: string, balanceX: number, balanceY: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "create-pool", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  addToPosition(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, pooltoken: string, dX: number, dY: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "add-to-position", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  reducePosition(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, pooltoken: string, percentage: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "reduce-position", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  swapXForY(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, dx: number, dy_min: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "swap-x-for-y", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  swapYForX(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, dy: number, dx_min: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "swap-y-for-x", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  getXgivenPrice(tokenX: string, tokenY: string, weightX: number, weightY: number, price: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-x-given-price", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  getYgivenPrice(tokenX: string, tokenY: string, weightX: number, weightY: number, price: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-y-given-price", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  setFeetoAddress(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, address: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-fee-to-address", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  getFeetoAddress(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "get-fee-to-address", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY)
  ...
  getFeeX(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "get-fee-rate-x", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY)
  ...
  getFeeY(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "get-fee-rate-y", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY)
  ...
  setFeeRateX(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, feerate:number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-fee-rate-x", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  setFeeRateY(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, feerate:number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-fee-rate-y", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  getYgivenX(tokenX: string, tokenY: string, weightX: number, weightY: number, dx: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-y-given-x", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  getXgivenY(tokenX: string, tokenY: string, weightX: number, weightY: number, dy: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-x-given-y", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  setFeeRebate(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, rebate : number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-fee-rebate", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),
  ...
  getFeeRebate(tokenX: string, tokenY: string, weightX: number, weightY: number) {
    return this.chain.callReadOnlyFn("fixed-weight-pool-ainomo", "get-fee-rebate", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(weightX),
      types.uint(weightY),
  ...
  setStartBlock(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, start_block: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-start-block", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),          
  ...
  setEndBlock(user: Account, tokenX: string, tokenY: string, weightX: number, weightY: number, end_block: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("fixed-weight-pool-ainomo", "set-end-block", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(weightX),
        types.uint(weightY),          
...
class FWPTestAgent3 {
  ...
  getPoolDetails(tokenX: string, tokenY: string) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-pool-details", [
      types.principal(tokenX),
      types.principal(tokenY)
    ], this.deployer.address);
  }
  ...
  getOracleResilient(tokenX: string, tokenY: string) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-oracle-resilient", [
      types.principal(tokenX),
      types.principal(tokenY)
    ], this.deployer.address);
  }    
  ...
  getOracleInstant(tokenX: string, tokenY: string) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-oracle-instant", [
      types.principal(tokenX),
      types.principal(tokenY)
    ], this.deployer.address);
  }      
  ...
  setOracleEnabled(user: Account, tokenX: string, tokenY: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-oracle-enabled", [
        types.principal(tokenX),
        types.principal(tokenY)
      ], user.address),
    ]);
  ...
  setOracleAverage(user: Account, tokenX: string, tokenY: string, average: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-oracle-average", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(average)
      ], user.address),
  ...
  addToPosition(user: Account, tokenX: string, tokenY: string, pooltoken: string, dX: number, dY: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "add-to-position", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.principal(pooltoken),
        types.uint(dX),
  ...
  reducePosition(user: Account, tokenX: string, tokenY: string, pooltoken: string, percentage: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "reduce-position", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.principal(pooltoken),
        types.uint(percentage),
  ...
  swapXForY(user: Account, tokenX: string, tokenY: string, dx: number, dy_min: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "swap-x-for-y", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(dx),
        types.some(types.uint(dy_min))
  ...
  swapYForX(user: Account, tokenX: string, tokenY: string, dy: number, dx_min: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "swap-y-for-x", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(dy),
        types.some(types.uint(dx_min))
  ...
  getXgivenPrice(tokenX: string, tokenY: string, price: number) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-x-given-price", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(price)
    ], this.deployer.address);
  ...
  getYgivenPrice(tokenX: string, tokenY: string, price: number) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-y-given-price", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(price)
    ], this.deployer.address);
  ...
  setFeetoAddress(user: Account, tokenX: string, tokenY: string, address: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-fee-to-address", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.principal(address) 
      ], user.address),
  ...
  getFeetoAddress(user: Account, tokenX: string, tokenY: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "get-fee-to-address", [
        types.principal(tokenX),
        types.principal(tokenY)
      ], user.address),
    ]);
  ...
  getFeeX(user: Account, tokenX: string, tokenY: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "get-fee-rate-x", [
        types.principal(tokenX),
        types.principal(tokenY)
      ], user.address),
    ]);
  ...
  getFeeY(user: Account, tokenX: string, tokenY: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "get-fee-rate-y", [
        types.principal(tokenX),
        types.principal(tokenY)
      ], user.address),
    ]);
  ...
  setFeeRateX(user: Account, tokenX: string, tokenY: string, feerate:number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-fee-rate-x", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(feerate)
      ], user.address),
  ...
  setFeeRateY(user: Account, tokenX: string, tokenY: string, feerate:number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-fee-rate-y", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(feerate)
      ], user.address),
  ...
  getYgivenX(tokenX: string, tokenY: string, dx: number) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-y-given-x", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(dx)
    ], this.deployer.address);
  ...
  getXgivenY(tokenX: string, tokenY: string, dy: number) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-x-given-y", [
      types.principal(tokenX),
      types.principal(tokenY),
      types.uint(dy)
    ], this.deployer.address);
  ...
  setFeeRebate(user: Account, tokenX: string, tokenY: string, rebate : number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-fee-rebate", [
        types.principal(tokenX),
        types.principal(tokenY),
        types.uint(rebate)
      ], user.address),
  ...
  getFeeRebate(tokenX: string, tokenY: string) {
    return this.chain.callReadOnlyFn("simple-weight-pool-ainomo", "get-fee-rebate", [
      types.principal(tokenX),
      types.principal(tokenY)
    ], this.deployer.address);
  }
  ...
  setStartBlock(user: Account, tokenX: string, tokenY: string, start_block: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-start-block", [
        types.principal(tokenX),
        types.principal(tokenY),    
        types.uint(start_block)
      ], user.address),
  ...
  setEndBlock(user: Account, tokenX: string, tokenY: string, end_block: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("simple-weight-pool-ainomo", "set-end-block", [
        types.principal(tokenX),
        types.principal(tokenY),      
        types.uint(end_block)
      ], user.address),
