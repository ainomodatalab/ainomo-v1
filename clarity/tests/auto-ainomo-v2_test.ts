...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that privileged setters can only be called by contract owner",
...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that contract is activated when adding to position",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const yieldVault = new YieldVault(chain, "auto-ainomo-v2");
...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that stacking is available when adding to position",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const yieldVault = new YieldVault(chain, "auto-ainomo-v2");
...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that add-to-position works on a valid pool",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        ...
        let block = chain.mineBlock([
            ...
            Tx.contractCall('auto-ainomo-v2', 'pause-create', [types.bool(false)], deployer.address),
            Tx.contractCall('auto-ainomo-v2', 'pause-redeem', [types.bool(false)], deployer.address)
        ]);
        for (let i = 0; i < block.receipts.length; i++) {
        ...
        block.receipts[0].events.expectFungibleTokenTransferEvent(
            dx,
            wallet_1.address,
            deployer.address + ".auto-ainomo-v2",
            "ainomo"
        );
        block.receipts[0].events.expectFungibleTokenTransferEvent(
            dx,
...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that claim-and-stake cannot claim future cycles",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const wallet_2 = accounts.get("wallet_2")!;
...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that claim-and-stake works with a valid cycle",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const wallet_2 = accounts.get("wallet_2")!;
...
Clarinet.test({
    name: "auto-ainomo-v2 / dual-farming-pool-v1-03 : ensure that claim-staking-reward-by-auto-ainomo works with a valid cycle",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const wallet_2 = accounts.get("wallet_2")!;
        ...
        block = chain.mineBlock([
            yieldVault.addToPosition(wallet_1, dx),
            // Tx.contractCall('dual-farming-pool-v1-03', 'claim-staking-reward-by-auto-ainomo', [types.principal(deployer.address + '.brc20-db20'), types.uint(0)], wallet_2.address)
        ]);
        ...
        block = chain.mineBlock([
            yieldVault.setBountyInFixed(wallet_2, BountyFixed),
            Tx.contractCall('dual-farming-pool-v1-03', 'claim-staking-reward-by-auto-ainomo', [types.principal(deployer.address + '.brc20-db20'), types.uint(1)], wallet_2.address)
            // yieldVault.claimAndStake(wallet_2, 1)
        ]);
        block.receipts.forEach(e => { e.result.expectErr() });
...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that claim-and-mint works with valid cycles",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const wallet_2 = accounts.get("wallet_2")!;
        ...
        let result: any = ainomoToken.mintFixed(deployer, wallet_1.address, 2 * dx);
        result.expectOk();
        ...
        block = chain.mineBlock([
            Tx.contractCall("auto-ainomo-v2", "claim-and-mint", [types.list([types.uint(0)])], wallet_1.address),
            Tx.contractCall("auto-ainomo-v2", "claim-and-mint", [types.list([types.uint(1), types.uint(2), types.uint(3)])], wallet_1.address)
        ]);
...
Clarinet.test({
    name: "auto-ainomo-v2 : ensure that reduce-position works",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const wallet_2 = accounts.get("wallet_2")!;
