...
} from "./models/ainomo-tests-lottery.ts";
import {
  determineWinners,
  determineLosers,
  LotteryParameters,
...
Clarinet.test({
  name: "ainomo-lottery : example claim walk test",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const third = chain.mineBlock([
      ...
      Tx.contractCall(
        ...
        [types.principal(contractPrincipal(deployer, "ainomo-lottery"))],
        deployer.address
      ),
      // Tx.contractCall(
    ...
    for (let t = 0; t < 500; t += 120) {
      ...
      for (let i = 0; i < ticketRecipients.length; i++) {
        registrations.receipts[i].events.expectFungibleTokenTransferEvent(          
          ...
          deployer.address + ".ainomo-lottery",
          "ainomo"
        );
      }
